...
def acessar_sistema():
    ...
    sistema = db.sistema(request.args(0, cast=int))
    if sistema.empresa!=usuario.empresa:
        if sistema.empresa_cliente!=usuario.empresa_cliente:
            redirect(URL('index'))
    return locals()
...
def cadastrar_atividade():
    ...
    db.registro_atividade.circunspecto.requires = IS_IN_DB(db(((db.usuario_empresa.empresa==usuario.empresa)or(db.usuario_empresa.empresa_cliente==sistema.empresa_cliente))&(db.usuario_empresa.ativo==True)), 'usuario_empresa.usuario', '%(nome)s')
